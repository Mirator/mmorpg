// Map editor runtime implementation
...
import { TEMPLATE_DEFINITIONS, filterTemplates, instantiateTemplate } from './templates.js';
import {
  BRUSH_SPACING,
  COLORS,
  DRAFT_KEY,
...
import { parseControlValue as parseControlFieldValue, nextId as nextSequenceId } from './actions.js';
import { bindMapEditorDom, createCanvasContexts } from './dom.js';
import {
  canvasPointFromMouse as canvasPointFromMouseEvent,
  updateCoordinateReadout as renderCoordinateReadout,
  updateMeasure,
} from './interactions.js';
import { createStatusRenderers } from './render.js';
import { hasSessionRestoreHint, setSessionRestoreHint } from './session.js';
...
} = bindMapEditorDom(document);
const { ctx, miniCtx } = createCanvasContexts(canvas, miniCanvas);
const { setStatus, setSaveStatus, setErrors, formatNumber: formatRenderedNumber } = createStatusRenderers({
  statusEl,
  saveStatusEl,
  errorsEl,
});
...
function formatNumber(value, digits = 2) {
  return formatRenderedNumber(value, digits);
}
...
function parseControlValue(input) {
  return parseControlFieldValue(input);
}
...
function nextId(list, prefix) {
  return nextSequenceId(list, prefix);
}
...
function canvasPointFromMouse(event) {
  return canvasPointFromMouseEvent(canvas, event);
}
